...
public class EvaluatePortfolioStateImpl extends EvaluatePortfolioState {
	...
	protected PortfolioStateBuilder doEvaluate(Portfolio input) {
		...
		Set<Position> aggregatedPositions = positionQuantity.keySet().stream()
				.map(p -> p.toBuilder()
						.setQuantity(Quantity.builder().setAmount(positionQuantity.get(p)))
...
//		}
	 ...
	 */
	private PositionStatusEnum toPositionStatus(TradeState tradeState, LocalDate date) {
	...
	private BigDecimal getAggregationSettlementAmount(TradeState tradeState) {
		...
		return tradeState.getTrade().getTradableProduct().getSettlementInstructions().stream()
				.findFirst()
				.map(PayoutBase::getSettlementTerms)
